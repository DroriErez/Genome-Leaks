...
def plotoneblock(A, REF=None, keys=[None, None], statname='LD', ax=None,
                 matshow_kws={'cmap': plt.get_cmap('viridis_r'), 'vmin': 0, 'vmax': 1}, suptitle_kws={}):
    if REF is None:
...
def plotLDblock(hcor_snp, left=None, right=None, ref='Real', mirror=False, diff=False, is_fixed=None, is_fixed_dic=None,
    ...
    if diff:
        ...
        cmap = plt.get_cmap('viridis_r')  # cool'
        cmap.set_bad('white')
...
def save_discriminator_class_pred(discriminator, test_dataset, experiment_results_path, id_to_class,
    ...
    for x_batch_real, y_batch_real in shuffled_dataset:
        ...
        class_metric_results.to_csv(os.path.join(experiment_results_path, DISCRIMINATOR_METRICS_FILE),
                                    index=False)
        del shuffled_dataset, x_batch_real, y_batch_real, r_f, class_predictions, y_pred
        del class_classifier_results, row_results
        gc.collect()
        return class_metric_results


def get_generated_class_counts(class_id_to_counts, total_generated_samples=None):
    if total_generated_samples is None:
        return {class_id: number_of_sequences * GENERATOR_POP_FACTOR
                for class_id, number_of_sequences in class_id_to_counts.items()}

    total_real_samples = sum(class_id_to_counts.values())
    generated_counts = {}
    remainders = []
    assigned_samples = 0
    for class_id, number_of_sequences in class_id_to_counts.items():
        exact_count = total_generated_samples * number_of_sequences / total_real_samples
        class_count = int(np.floor(exact_count))
        generated_counts[class_id] = class_count
        assigned_samples += class_count
        remainders.append((exact_count - class_count, class_id))

    for _, class_id in sorted(remainders, reverse=True)[:total_generated_samples - assigned_samples]:
...
def generate_fake_samples(class_id_to_counts, epoch_number, generator, id_to_class, latent_size, num_classes,
                          sequence_results_path, framework, total_generated_samples=None):
    generated_genomes_total = []
    generated_class_counts = get_generated_class_counts(class_id_to_counts, total_generated_samples)
    for class_id, number_of_generated_sequences in generated_class_counts.items():
        if number_of_generated_sequences == 0:
            continue
        ...
        del fake_labels_batch, latent_samples, generated_genomes, tmp_generated_genomes_df
...
def plot_pca_comparisons(generator: Model, epoch_number: int,
                         ...
                         framework="tensorflow", total_generated_samples=None, generated_genomes_df=None):
    """
    ...
    """
    # Create AGs
    if generated_genomes_df is None:
        generated_genomes_df = generate_fake_samples(class_id_to_counts, epoch_number, generator, id_to_class,
                                                     latent_size, num_classes, sequence_results_path, framework,
